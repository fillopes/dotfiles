...
#echo "Eu sou o relâmpago marquinhos, vruuuuuuuuuuuum vruuuuuuuuuuum"
...
export VISUAL="${EDITOR:-/usr/bin/vim}"
if [ "$UID" != 0 ]; then
    export XPAGER=$PAGER
...
show_time() {
  echo "${PALE_GREEN}[$(date +%H:%M)]${GT_RESET}"
}
...
# Define prompt based on whether I am local or connected via ssh
        ...
        PS1='$(show_time) $(prompt_color)'"\h${COLOR_NONE}:${PALE_BLUE}\w${GT_RESET}"' $(show_exit_status)'"${GT_RESET}\n$(prompt_symbol) "
